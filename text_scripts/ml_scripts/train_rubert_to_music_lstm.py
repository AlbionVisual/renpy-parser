...
class LSTMRegressor(nn.Module):
    ...
    def forward(self, x_padded: torch.Tensor, lengths: torch.Tensor) -> torch.Tensor:
        ...
        out_packed, _h = self.lstm(packed)
        out, _out_lens = pad_packed_sequence(out_packed, batch_first=True)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="LSTM (PyTorch): RuBERT -> music_data, sequence per game.",
    )
...
def main() -> int:
    ...
    if args.embedding_lags < 0:
        ...
        return 2
    if args.lstm_hidden < 1:
        print("--lstm-hidden must be >= 1")
        return 2
    if args.lstm_layers < 1:
        print("--lstm-layers must be >= 1")
        return 2
    ...
    with RunArtifacts(getattr(args, "run_dir", None)):
        with training_db_context(args) as conn:
            ...
            model = LSTMRegressor(
                in_dim=in_dim,
                hidden_dim=int(args.lstm_hidden),
            ...
            print(
                ...
                out_dim,
                "lstm_hidden",
                int(args.lstm_hidden),
                "lstm_layers",
                int(args.lstm_layers),
                "params",
                n_params,
            )
            ...
            train_metrics_summary = print_regression_block(
                "LSTM Regression metrics ON TRAIN (fit set, denormalized Y)",
                loaded.y_columns,
            ...
            if val_idx:
                ...
                val_metrics_summary = print_regression_block(
                    "LSTM metrics ON VALIDATION (denormalized Y)",
                    loaded.y_columns,
                    loaded.y[val_idx],
                    y_val_pred,
                    False,
            ...
            test_metrics = print_regression_block(
                "LSTM Regression metrics ON TEST ONLY",
                loaded.y_columns,
                loaded.y[test_idx],
                y_pred,
                False,
            ...
            if args.per_column_metrics_csv is not None:
                write_per_column_metrics_csv(
                    ...
                    meta={
                        ...
                        "lstm_hidden": str(int(args.lstm_hidden)),
                        "lstm_layers": str(int(args.lstm_layers)),
