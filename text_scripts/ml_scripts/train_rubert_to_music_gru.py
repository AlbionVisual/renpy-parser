...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="GRU (PyTorch): RuBERT -> music_data, sequence per game.",
    )
...
def main() -> int:
    ...
    if args.embedding_lags < 0:
        ...
        return 2
    if args.gru_hidden < 1:
        print("--gru-hidden must be >= 1")
        return 2
    if args.gru_layers < 1:
        print("--gru-layers must be >= 1")
        return 2
    ...
    with RunArtifacts(getattr(args, "run_dir", None)):
        with training_db_context(args) as conn:
            ...
            model = GRURegressor(
                in_dim=in_dim,
                hidden_dim=int(args.gru_hidden),
                num_layers=int(args.gru_layers),
            ...
            train_metrics_summary = print_regression_block(
                "GRU Regression metrics ON TRAIN (fit set, denormalized Y)",
                loaded.y_columns,
            ...
            if val_idx:
                ...
                val_metrics_summary = print_regression_block(
                    "GRU metrics ON VALIDATION (denormalized Y)",
                    loaded.y_columns,
                    loaded.y[val_idx],
                    y_val_pred,
                    False,
            ...
            test_metrics = print_regression_block(
                "GRU Regression metrics ON TEST ONLY",
                loaded.y_columns,
                loaded.y[test_idx],
                y_pred,
                False,
            ...
            if args.per_column_metrics_csv is not None:
                write_per_column_metrics_csv(
                    ...
                    meta={
                        ...
                        "gru_hidden": str(int(args.gru_hidden)),
                        "gru_layers": str(int(args.gru_layers)),
