...
class CausalConvBlock(nn.Module):
    def __init__(self, cin: int, cout: int, kernel: int, dilation: int, dropout: float) -> None:
        ...
        self.conv = nn.Conv1d(cin, cout, self.kernel, dilation=self.dilation)
...
class TCNRegressor(nn.Module):
    def __init__(
        ...
        *,
        ...
        for i in range(lv):
            d = 2**i
            blocks.append(CausalConvBlock(c, ch, k, d, dropout))
            c = ch
...
def collate_game_seqs(
    ...
    lengths = torch.tensor([int(x.shape[0]) for _g, x, _y, _idx in batch], dtype=torch.int64)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="TCN (causal dilated Conv1d): RuBERT -> music_data, sequence per game.",
    )
    ...
    p.add_argument("--tcn-levels", type=int, default=6)
...
def main() -> int:
    ...
    if args.embedding_lags < 0:
        ...
        return 2
    if args.tcn_channels < 1:
        print("--tcn-channels must be >= 1")
        return 2
    if args.tcn_levels < 1:
        print("--tcn-levels must be >= 1")
        return 2
    if args.tcn_kernel < 2:
        print("--tcn-kernel must be >= 2")
        return 2
    ...
    with RunArtifacts(getattr(args, "run_dir", None)):
        with training_db_context(args) as conn:
            ...
            if args.split == "within_game":
                train_idx, test_idx = split_within_game(loaded.rows, args.test_fraction, args.seed)
            else:
                if args.held_out_games_seed is None:
                    ...
                    print("held_out_games_seed fixed", int(args.held_out_games_seed))
            ...
            if not train_idx or not test_idx:
                print("empty train or test split", "train", len(train_idx), "test", len(test_idx))
                return 2
            ...
            print_split_visibility(loaded.rows, train_idx, test_idx, args, held_out_games)
            ...
            if x_fit_mean is not None:
                x_all_n = ((x_lag.astype(np.float64) - x_fit_mean) / x_fit_std).astype(np.float32)
            else:
            ...
            if args.no_standardize_y:
                ...
                y_all_n = ((loaded.y.astype(np.float64) - y_mean) / y_std).astype(np.float32)
    
            game_train = _by_game_ordered_indices(loaded.rows, tune_train_idx)
            game_val = _by_game_ordered_indices(loaded.rows, val_idx) if val_idx else {}
            game_test = _by_game_ordered_indices(loaded.rows, test_idx)
    
            ds_train = GameSeqDataset(rows=loaded.rows, x_all=x_all_n, y_all=y_all_n, game_to_indices=game_train)
            ds_val = GameSeqDataset(rows=loaded.rows, x_all=x_all_n, y_all=y_all_n, game_to_indices=game_val) if val_idx else None
            ds_test = GameSeqDataset(rows=loaded.rows, x_all=x_all_n, y_all=y_all_n, game_to_indices=game_test)
            ...
            def predict_all_rows(loader: DataLoader) -> dict[int, np.ndarray]:
                ...
                with torch.inference_mode():
                    for _games, xb, _yb, lengths, mask, indices in loader:
                        xb = xb.to(device)
                        lengths = lengths.to(device)
                        pred = model(xb, lengths).detach().cpu().numpy().astype(np.float32)
                        mask_np = mask.numpy()
            ...
            y_pred_n = np.stack([pred_map_test[i] for i in test_idx], axis=0).astype(np.float32)
            if args.no_standardize_y:
                y_pred = y_pred_n.astype(np.float32)
            else:
                y_pred = (y_pred_n.astype(np.float64) * y_std + y_mean).astype(np.float32)
            ...
            y_train_pred_n = np.stack([pred_map_train[i] for i in tune_train_idx], axis=0).astype(np.float32)
            if args.no_standardize_y:
                y_train_pred = y_train_pred_n.astype(np.float32)
            else:
                y_train_pred = (y_train_pred_n.astype(np.float64) * y_std + y_mean).astype(np.float32)
    
            train_metrics_summary = print_regression_block(
                "TCN Regression metrics ON TRAIN (fit set, denormalized Y)",
                loaded.y_columns,
            ...
            if val_idx:
                pred_map_val = predict_all_rows(val_loader) if val_loader is not None else {}
                y_val_pred_n = np.stack([pred_map_val[i] for i in val_idx], axis=0).astype(np.float32)
                if args.no_standardize_y:
                    y_val_pred = y_val_pred_n.astype(np.float32)
                else:
                    y_val_pred = (y_val_pred_n.astype(np.float64) * y_std + y_mean).astype(np.float32)
                val_metrics_summary = print_regression_block(
                    "TCN metrics ON VALIDATION (denormalized Y)",
                    loaded.y_columns,
                    loaded.y[val_idx],
                    y_val_pred,
                    False,
            ...
            test_metrics = print_regression_block(
                "TCN Regression metrics ON TEST ONLY",
                loaded.y_columns,
                loaded.y[test_idx],
                y_pred,
                False,
            ...
            if args.per_column_metrics_csv is not None:
                write_per_column_metrics_csv(
                    ...
                    meta={
                        ...
                        "tcn_channels": str(int(args.tcn_channels)),
                        "tcn_levels": str(int(args.tcn_levels)),
                        "tcn_kernel": str(int(args.tcn_kernel)),
                ...
                print("wrote per-column metrics csv", str(args.per_column_metrics_csv))
            ...
            cands = load_candidates_dir(args.dataset_dir) if args.dataset_dir is not None else None
